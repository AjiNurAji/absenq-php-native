...
<div class="container mx-auto">
  <!-- cancel add -->
  <a href="/courses" class="mb-4 text-white bg-red-600 px-4 py-1 w-fit block text-sm rounded-lg">Batal</a>
  <div class="overflow-hidden bg-white p-4 rounded-lg shadow">
    <form id="course-form" class="text-left">
      <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nama Matkul</label>
      <input type="text" name="name" id="name"
        ...
        placeholder="Masukkan matkul" required value="<?= $course->course_name ?>" />
...
<script>
  ...
  addForm.addEventListener("submit", async (e) => {
    ...
    const res = await fetch("/course/edit/<?= $course->id ?>", {
      method: "POST",
      headers: {
