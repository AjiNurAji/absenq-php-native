  ...
  <div class="overflow-hidden bg-white p-4 rounded-lg shadow">
    <form id="student-form" class="text-left">
      ...
      <input type="text" name="id_number" id="id_number"
        class="w-full p-3 border border-gray-300 rounded-lg mb-4 placeholder:text-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
        placeholder="Masukkan NIM" value="<?= htmlspecialchars($data->student_id) ?>" disabled required />
      ...
      <input type="text" name="name" id="name"
        ...
        placeholder="Masukkan nama lengkap" value="<?= htmlspecialchars($data->name) ?>" required />
...
<script>
  ...
  addForm.addEventListener("submit", async (e) => {
    ...
    const res = await fetch("/student/update/<?= $data->student_id ?>", {
      method: "POST",
      headers: {
